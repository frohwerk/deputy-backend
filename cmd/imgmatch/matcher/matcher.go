...
func init() {
	Log.Warn("cmd/imgmatch/matcher/matcher.go - TODO: Replace static fsignore with customizable one")
}
	...
	archiveloop:
		for _, archive := range archives {
			...
			for j, k := 0, i+matches; j < len(archive.Files) && k < len(fs); {
				...
				Log.Trace("Matching file %s with %s", name, fs[k].Name)
				switch {
				case name > fs[k].Name:
					k++
				case name < fs[k].Name:
				...
				case archive.Files[j].Digest != fs[k].Digest:
					Log.Trace(">>>> Unmatched file: %s from archive '%s' not found in image", name, archive.File.Name)
					scanned.Put(archive.Id)
					continue archiveloop
				default:
					Log.Trace("Found file %s at path %s", archive.Files[j].Name, path)
					matches++
					k++
					j++
				}
			...
			if matches == len(archive.Files) {
				Log.Info("Found all files, the archive '%s' is a match", archive.Id)
				scanned.Put(archive.Id)
				if matches > strength {
					strength, matches = matches, 0
					result = []database.File{archive.File}
				} else {
					Log.Info("Ignoring archive '%s', since '%s' is a better match", archive.Id, result[0].Id)
				}
			} else {
				Log.Info(">>>> Archive '%s' has %v unmatched files", archive.Id, len(archive.Files)-matches)
			}
		}
	}
	return result, nil
